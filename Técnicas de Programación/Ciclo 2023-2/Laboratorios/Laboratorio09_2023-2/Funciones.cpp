...
// Módulo que intercambia dos estructuras de tipo "Curso" usando una variable auxiliar.
...
// Módulo que ordena el arreglo de cursos de forma ascendente por el código del curso.
...
// Módulo que intercambia dos estructuras de tipo "Alumno" usando una variable auxiliar
...
// Módulo que ordena un arreglo de alumnos de forma ascendente por el código del alumno.
...
// Módulo que ordena las listas de alumnos de cada curso dentro del arreglo de cursos.
...
// Módulo que ordena, primero, el arreglo de cursos y, luego, las listas de alumnos dentro de cada curso.
void ordenar_arreglo_de_cursos_y_listas_de_alumnos(struct Curso *arrCursos, int cantCursos) {
    ordenar_cursos(arrCursos, cantCursos);             // Ordenar los cursos por código de curso.
    ordenar_listas_de_alumnos(arrCursos, cantCursos);  // Ordenar los alumnos en cada curso por código de alumno.
}
...
void completar_campos_restantes_del_alumno(struct Alumno &un_alumno, char *nombre_alumno,
    ...
    if(indice_escala != NO_ENCONTRADO) {
        ...
        un_alumno.pagoPorCredito = 0.0; // No se encontró, entonces se asigna el pago por crédito en cero.
    }
}
