    ...
    while(true){
        ...
        if(cod_medico_que_atendio == codigo_medico and (tiempo_final - tiempo_inicial) < (2*3600+31*60+25)){
            ...
            if(input_citas.get() != '\n'){  // Preguntar si llegó al fin de línea; en caso sí, pues no le recetaron medicinas.
                ...
                procesar_lista_de_medicinas(pago_total_por_medicinas,input_citas,input_medicinas,output,porcentaje_descuento_cliente);
            } else {
...
void procesar_lista_de_medicinas(double &pago_total_por_medicinas,ifstream &input_citas,
                                 ifstream &input_medicinas,ofstream &output,double porcentaje_descuento_cliente){
    int codigo_medicina,cantidad,cant_medicinas=0;
    elaborar_encabezados(3,output);
    while(true){
        input_citas >> codigo_medicina >> cantidad;
        imprimir_medicina(codigo_medicina,cantidad,cant_medicinas,pago_total_por_medicinas,
                          input_medicinas,output,porcentaje_descuento_cliente);
        if(input_citas.get() == '\n') break;
    }
}
/*****************************************************************************************************************************************/
...
void imprimir_medicina(int codigo_medicina,int cantidad,int &cant_medicinas,
                       double &pago_total_por_medicinas,ifstream &input,
                       ofstream &output,double porcentaje_descuento_cliente){
    ...
    while(true){
        ...
        if(cod_medicina_leida == codigo_medicina){
            ...
            output << (precio * ( (100 - (porcentaje_descuento_cliente/2)) / 100) ) * cantidad << endl;
            pago_total_por_medicinas += (precio * ( (100 - (porcentaje_descuento_cliente/2)) / 100) ) * cantidad;
            break;
        } else {
            while(input.get() != '\n'); // Descartar medicina
        }
